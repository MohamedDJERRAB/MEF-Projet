...
class Assemblage:
	...
	def Loc2Glob(self,p,i):
		if i <= 0 and i > 3:
			raise Exception("Le sommet n'est pas compris entre 1 et 3")

		if self.liste_element[p-1][1] == 1 and i >= 3:
			raise Exception("Le sommet locale n'est pas inférieur ou égal à 2 pour un segment!")
		return self.liste_element[p-1][2+i]
		...
		for p in range(self.non_tri+1,self.nbr_elements+1):
			...
			for i in range(3):
				...
				for j in range(3):
					...
					ligne.append(I)
					colonne.append(J)
		self.M = coo_matrix((np.array(data)*k*k,(ligne,colonne))).tocsr()
	...
	def Four_Robin(self):
		...
		self.MFR = coo_matrix((np.array(data)*np.complex(0,-k),(ligne,colonne)),shape=(self.nbr_nodes,self.nbr_nodes),dtype=complex).tocsr()



	def u_in(self,x,y):
		a= 0
		return np.exp(np.complex(0,1)*k*(x*np.cos(a) + y*np.sin(a)))
		...
		for index,elem in enumerate(self.liste_element):
			if elem[1] ==3:
				for i in elem[2:]:
					self.A[i-1,:]=0
					self.A[i-1,i-1]=1
					self.b[i-1] = -self.u_in(self.liste_nodes[i-1][0],self.liste_nodes[i-1][1])
